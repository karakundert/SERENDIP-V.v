...
___________________________________________________________________________
 modification history:
   ...
   version 2.0 Copied over from SVN on 21Jun12 for SERENDIP V database
      code by Laura and quickly hacked to remove all references to 
      receiver ID
___________________________________________________________________________
...
double seti_dop_FreqOffsetAtBaryCenter(double DetectionFreq, double JulDay, double Ra, double Dec, 
                                    double Epoch, double Latitude, double Longitude, double Elevation) {
...
double seti_dop_FreqAtBaryCenter(double DetectionFreq, double JulDay, double Ra, double Dec, 
                              double Epoch, double Latitude, double Longitude, double AOElevation) {
...
//  ReceiverConfig_t	ReceiverConfig;
...
*/
   ...
   if(JulDay - (int)JulDay >= 0.5) {			// jd is for 0.0hr, UT
    ...
    Elevation = AOElevation / 1000.0;	// elevation in Km
    ...
    seti_dop_topov(seti_dop_lmst(jd, ut, -1.0*Longitude), 
                Latitude, Elevation, vt);  // observer diurnal motion
