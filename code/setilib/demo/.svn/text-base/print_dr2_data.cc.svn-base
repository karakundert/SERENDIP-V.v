...
int main(int argc, char ** argv) {
    ...
    for (channel=0; channel < num_channels; channel++) {
    	...
    	retval = seti_GetDr2Data(fd, channels[channel], num_samples, cs, NULL, cw, 0, 
                                 show_blanking_signal);
        if(!minus_one) {
    ...
    if (graphics) {
        ...
        for (sample = 0; sample < num_samples; sample++) {
            ...
            if (show_blanking_signal) {
                fprintf(stderr,"%2d %2d ", (int)cw[sample].real(), (int)cw[sample].imag());
            }
       	    fprintf(stderr,"\n");
        }
    }
